...
public class MontrealClass {
	...
	public void udp_packet_recv() {
		DatagramSocket aSocket = null;
		int portNo = Constants.RM2_MTL_PORT_NO;
		try {
			aSocket = new DatagramSocket(portNo);
			byte[] buffer = new byte[1000];// to stored the received data from
											// the client.
			System.out
					.println("RM 2: Montreal UDP Server Started on port: " + portNo);
				...
				if (keyevent.equalsIgnoreCase(eventType)) {
					for (String event : eventColl.get(keyevent)) {
						if (event.equalsIgnoreCase(eventID)) {
							montr_db.get(eventType).get(eventID).setBookingCapacity(montr_db.get(eventType).get(eventID).getBookingCapacity()+1);
