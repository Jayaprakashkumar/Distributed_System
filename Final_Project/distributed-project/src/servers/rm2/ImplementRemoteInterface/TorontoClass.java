package servers.rm2.ImplementRemoteInterface;
...
import servers.rm2.common.EventDetails;
import utility.Constants;

public class TorontoClass  {
	...
	public String udp_packet_send(String destServer, String actionType,
		...
		try {
			...
			if (destServer.equals("MTL")) {
				serverPort = Constants.RM2_MTL_PORT_NO;
			} else if (destServer.equals("OTW")) {
				serverPort = Constants.RM2_OTW_PORT_NO;
			} else {
	...
	public void udp_packet_recv() {
		DatagramSocket aSocket = null;
		int portNo = Constants.RM2_TOR_PORT_NO;
		try {
			aSocket = new DatagramSocket(portNo);
			byte[] buffer = new byte[1000];// to stored the received data from
											// the client.
			System.out.println("RM 2: Toronto UDP Server Started on port: " + portNo);
	...
	public synchronized boolean addEvent(String eventID, String eventType,
...
//					+ "No events found with event type: " + eventType;
			disp_events = eventType + " - " + "No events found";
			message = " Manager ID : " + managerID + " , " + "Event type : "
					+ eventType + " , "
					+ "Action Performed : listEventAvailability" + " , "
		...
		if (user_db.containsKey(customerID)) {
			...
			for (String keyevent : eventType.keySet()) {
				for (String event : eventType.get(keyevent)) {
					finalEvents = finalEvents + event + " "+ keyevent+ ",";
				}
			}
	...
	public synchronized boolean cancelEvent(String customerID, String eventID,
			String eventType) {
				...
				if (keyevent.equalsIgnoreCase(eventType)) {
					for (String event : eventColl.get(keyevent)) {
						if (event.equalsIgnoreCase(eventID)) {
							toronto_db.get(eventType).get(eventID).setBookingCapacity(toronto_db.get(eventType).get(eventID).getBookingCapacity()+1);
	...
	public synchronized boolean swapEvent(String customerID, String newEventID,
		...
		if (cancelEvent(customerID, oldEventID, oldEventType))
		{
			...
			{
				message = " Customer ID :" + customerID + " , " + " New EventId : "
						+ newEventID + " , " + "New Event Type " + newEventType + " , "
						+ "Old Event Id " + oldEventID + " , " + "Old Event Type " + oldEventType + " , "
						+ " Action Performed : Swap Event  " + " , "
						+ " Message : Event swapped successfully\n";
				Helpers.usersLog(customerID, message);
				Helpers.ServerLog("TOR", message);
