...
public class RepManager2 {
	...
	private synchronized static void udpReceive() {
		...
		try {
			// aSocket = new DatagramSocket(Constants.RM2_PORT_NO);
			...
			System.out.println("-----------------------------------");
			System.out.println("RM 2 Server listening on multicast port " + Constants.MULTICAST_PORT_NO);
		...
		if (receivedParams[1].substring(0, 3).equalsIgnoreCase("MTL")) {
			...
			case "addEvent":
				ret = montreal_obj.addEvent(receivedParams[3], receivedParams[4], Integer.parseInt(receivedParams[5]),
						receivedParams[1]);
				udpSend("RM2", ret + ";" + receivedParams[0] + ";" + receivedParams[2]);
				break;
			case "removeEvent":
			...
			case "listEventAvailability":
				...
				udpSend("RM2", retStr + ";" + receivedParams[0] + ";" + receivedParams[2]);
				break;
			case "bookEvent":
				/* Add requests in queue */
				dataList.add(receivedParams);
				if (!Boolean.parseBoolean(receivedParams[5])) {
					for (String[] tempParams : dataList) {

						ret = montreal_obj.bookEvent(tempParams[1], tempParams[3], tempParams[4]);
						udpSend("RM2", ret + ";" + tempParams[0] + ";" + receivedParams[2]);
					}
