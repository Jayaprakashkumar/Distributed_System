...
public class MontrealServerImplementation extends UnicastRemoteObject implements ClientServerInterface {

	private static Logger LOGGER = LogManager.getLogger("temp");
	...
	public MontrealServerImplementation() throws RemoteException {
		super();
	}
			...
			try {
				...
				while (true) {
					...
					else if(eventChoice.equalsIgnoreCase("bookEvent"))
					{
						...
						outputRes = (MtlServer.BookEvent(splitRecievedStr[0], splitRecievedStr[1], splitRecievedStr[2]).equalsIgnoreCase("success")) ? "active":"inactive";
						sendDataByte = outputRes.getBytes();
					}
					else if(eventChoice.equalsIgnoreCase("getBooking"))
					{
						System.out.println("Get bookin Schedule: Inter server communication");
						LOGGER.info("Get bookin Schedule: Inter server communication");
					...
					else if(eventChoice.equalsIgnoreCase("cancelEvent"))
					{
						...
						MtlServer.CancelEvent(splitRecievedStr[0],splitRecievedStr[1], splitRecievedStr[2]);
						outputRes = "Calling Cancelling Event";
						sendDataByte = outputRes.getBytes();
	...
	public synchronized String BookEvent(String customerId, String eventId, String eventType) throws RemoteException {
		...
		if (city.equalsIgnoreCase("mtl")) {
			...
			try {
				...
				customerBookingAccess.put(customerId,count + 1);
				System.out.println(" Oustide city count : "+customerBookingAccess.get(customerId));
				LOGGER.info(" Oustide city count : "+customerBookingAccess.get(customerId));
